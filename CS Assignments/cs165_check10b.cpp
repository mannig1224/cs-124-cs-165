...
void promptInventory(vector<Food*> &items)
{
   ...
   do
   {
      ...
      if (name != "quit")
      {
         ...
         Food *myFood = new Food(name, month, year);
         items.push_back(myFood);
...
void displayInventory(vector<Food*> &items)
{
   ...
   for (vector<Food*>::iterator
   myIt = items.begin(); myIt != items.end(); ++myIt)
   {
     (*myIt)->display();
   }
                        ...
                        int currentYear)
{
   ...
   vector<Food*>::iterator myIt = items.begin();
   while (myIt != items.end())
   {
     if((*myIt)->getYear() < currentYear || ((*myIt)->getYear() == currentYear &&
     (*myIt)->getMonth() < currentMonth))
     {
        delete *myIt;
        myIt = items.erase(myIt);
     }
     else
     {
       myIt++;
